...
import com.wesco.ordermgmt.IntegrationTest;
...
class CustomerOrderMangmentKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/customer-order-mangment-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/customer-order-mangment-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/customer-order-mangment-kafka/unregister"));
                return;
            }
        }
